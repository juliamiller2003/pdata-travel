...
country_code must be a valid ISO 3166-1 alpha-2 code. estimated_cost is the TOTAL trip cost in USD as an integer, including flights from ${location}, accommodation, food, and activities for ${duration} days. estimated_flights is the round-trip flight cost from ${location} in USD as an integer.${budget ? ` Only suggest destinations reachable within the $${budget} budget.` : ""}${travelDate ? ` Prioritize destinations with good weather, low crowds, or notable events during the specified travel month.` : ""}`;
